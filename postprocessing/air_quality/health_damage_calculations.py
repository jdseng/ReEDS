...
1. called automatically from runreeds.py as part of a ReEDS run,
...
def get_marginal_damage_rates(casepath):
    ...
    fpath = Path(reeds.io.reeds_path, 'postprocessing', 'air_quality', 'rcm.h5')
    mds = reeds.io.read_h5_groups(fpath)
    ...
    mds_avg = mds_mapped.groupby(['ba', 'model', 'cr', 'pollutant'])['damage'].mean()
